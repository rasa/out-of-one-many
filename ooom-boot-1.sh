...
# create and partition disks per ooom.fstab file - first boot
...
ooom::install() {
  ...
  local entry
  for entry in $OOOM_PACKAGE_MAP; do
    local fs="${entry%%,*}"
    local package="${entry#*,}"
...
ooom::mkfs() {
  ...
  if [[ "${fmt}" = "swap" ]]; then
    ooom::mkswap "${devn}" "${vol}" "${fmt}" "${opt}" "${ex1}" "${ex2}" "${uuid}"
    return
  fi
  ...
  local voldir="$(echo ${vol} | tr -d /)"

  local mnt="${OOOM_MOUNT}/${vol}dir"
  ...
  local EL=$? ; test "$EL" -gt 0 && echo "*** Error: Command returned error $EL"
  ...
  if egrep -v '^\s*#' /etc/fstab.pre-ooomed | tr -s "\t" " " | cut -d' ' -f 2 | egrep -q "^${vol}$"; then
    ...
    perl -pi.${vol}dir.ooomed -e "s|^\s*(\S+\s+${vol}\s+.*)$|#\1|;" /etc/fstab
  fi
...
if [[ -f "${OOOM_DIR}/ooom-custom-boot-1-start.sh" ]]; then
...
cat "${OOOM_FSTABS}" | while IFS=$' \t' read -r -a var; do
  ...
  ooom::mkfs "${dev}" "${vol}" "${fmt}" "${opt}" "${ex1}" "${ex2}" "${uuid}"
done

echo === Step 4: ooom::mkvol
...
if [[ "${OOOM_BOOT_VOL}" ]]; then
  echo === Step 6: ooom::setboot
  ...
  cat "${OOOM_FSTABS}" | while IFS=$' \t' read -r -a var; do
    ...
    if [[ "${vol}" = "${OOOM_BOOT_VOL}" ]]; then
      ooom::setboot "${dev}" "${vol}"
      break
    fi
  done
fi

if [[ "$OOOM_GRUB_VOL" ]]; then
  echo === Step 7: ooom::installgrub
  ...
  cat "${OOOM_FSTABS}" | while IFS=$' \t' read -r -a var; do
    ...
    if [[ "${vol}" = "$OOOM_GRUB_VOL" ]]; then
      ooom::installgrub "dev" "${vol}"
      break
    fi
  done
fi
