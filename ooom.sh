...
# create and partition disks per ooom.fstab file
...
if [[ ! -d "${OOOM_LOG_DIR}" ]]; then
  mkdir -p "${OOOM_LOG_DIR}"
fi

LOG="${OOOM_LOG_DIR}/ooom.log"

if [[ -f "${OOOM_DIR}/ooom-custom-start.sh" ]]; then
